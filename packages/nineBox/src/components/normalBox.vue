<template>
    <div class="zt-ninebox" >
        ...
        <ul>
            <li v-for="(item,index) in dataItems" :class="parseInt(index+1) % columnNum == 0 ? 'noRightBorder': ''" :style="{width: parseInt(index+1) % columnNum == 0 ? `${width - liWidth*(columnNum-1)}%`: `${liWidth}%`, height:`${parseInt(height/columnNum)}rem`}">
